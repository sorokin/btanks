/* d3dsdl - libSDL direct3d backed/wrapper for the SDL, version for the DX 8.1
...
SDL_Surface *d3dSDL_SetVideoMode(int width, int height, int bpp, Uint32 flags) {
    ...
    else
    {
	    ...
	    //d3dpp.PresentationInterval   = D3DPRESENT_INTERVAL_IMMEDIATE; // Do NOT sync to vertical retrace (dx9)
    }
...
static SDL_Surface * convert_to_display(SDL_Surface *surface, bool alpha_channel) {
	...
	info.tex = new LPDIRECT3DTEXTURE8[info.n];
	int idx = 0;
	...
	r->unused1 = (Uint32)g_textures.size();
...
int d3dSDL_SaveBMP(SDL_Surface *surface, const char *file) {
	...
	if (surface == g_screen) {
		...
		if (FAILED(g_pd3dDevice->CreateImageSurface(surface->w, surface->h, D3DFMT_A8R8G8B8, &dst))) {
			SDL_SetError("CreateImageSurface failed");
			return -1;
		}
		
		if (FAILED(g_pd3dDevice->GetFrontBuffer(dst))) {
