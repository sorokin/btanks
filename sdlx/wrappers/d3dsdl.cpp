...
int d3dSDL_SaveBMP(SDL_Surface *surface, const char *file) {
	...
	if (surface == g_screen) {
		...
		if (FAILED(g_pd3dDevice->CreateOffscreenPlainSurface(surface->w, surface->h, D3DFMT_A8R8G8B8, D3DPOOL_SCRATCH, &dst, nullptr))) {
			...
			for(int y = y1; y < y2; ++y) {
				...
				for(int x = x1; x < x2; ++x) {
					...
					*/
					...
					if (FAILED(g_sprite->Draw(tex->tex[idx], &src_rect, nullptr, &pos, D3DCOLOR_RGBA(0xff, 0xff, 0xff, alpha)))) {
