...
void Object::tick(const float dt) {
	if (clunk_object != nullptr) {
		...
		v2<float> pos = Map->distance(v2<float>(listener_pos.x, listener_pos.y), get_center_position());
		clunk_object->update(clunk::v3<float>(pos.x, -pos.y, 0), clunk::v3<float>(_velocity.x, -_velocity.y, 0));
	}
